...
class ControllerExtensionModuleBhaveshMegamenu extends Controller {
    ...
    public function index() {
        ...
        $this->load->language('extension/module/bhavesh_megamenu');
        $this->document->setTitle($this->language->get('heading_title'));
        $this->load->model('setting/setting');
        $this->load->model('extension/bhavesh/bhavesh_megamenu');
        $this->load->model('tool/image');
        $this->load->model($model_module_load);
        $this->document->addStyle('view/javascript/bhavesh_megamenu/bhavesh_megamenu.css');
        $this->document->addScript('view/javascript/bhavesh_megamenu/jquery.nestable.js');
		...
		if(isset($_GET['delete'])) {
            if($this->validate()){
                if($this->model_extension_bhavesh_bhavesh_megamenu->deleteMenu(intval($_GET['delete']))) {
            ...
            $this->response->redirect(HTTPS_SERVER . 'index.php?route=extension/module/bhavesh_megamenu&'.$token_prefix.'=' . $this->session->data[$token_prefix].'&module_id='.$this->request->get['module_id']);
        }
            ...
            if(isset($_POST['button-create'])) {
                if($this->validate()) {
                    ...
                    if($error == true) {
                        ...
                        $this->model_extension_bhavesh_bhavesh_megamenu->addMenu($this->request->post);
            ...
            if(isset($_POST['button-edit'])) {
                if($this->validate()) {
                    ...
                    if($error == true) {
						...
						if(isset($this->request->post['content']['subcategory']['category']) && ($this->request->post['content']['subcategory']['category']))
						$this->request->post['content']['subcategory']['category'] = json_encode($this->request->post['content']['subcategory']['category'],true);						
                        $this->model_extension_bhavesh_bhavesh_megamenu->saveMenu($this->request->post);
        ...
        $data['nestable_list'] = $this->model_extension_bhavesh_bhavesh_megamenu->generate_nestable_list($data['language_id']);
        if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
            if(isset($_POST['button-save'])){
				...
				$moduleid_new= $this->model_extension_bhavesh_bhavesh_megamenu->getModuleId(); // Get module id
				$module_id = '';
				if (!isset($this->request->get['module_id'])) {
					$this->request->post['moduleid'] = $moduleid_new[0]['Auto_increment'];
					$module_id = $moduleid_new[0]['Auto_increment'];
					$this->$model_module_path->addModule('bhavesh_megamenu', $this->request->post);	
				} else {
				...
				if (isset($this->request->post['import_module']) && $this->request->post['import_module']) {
					...
					$this->model_extension_bhavesh_bhavesh_megamenu->duplicateModule($module_id,$import_module);
				}					
                $this->session->data['success'] = $this->language->get('text_success');
				$this->remove_cache();
				$this->response->redirect($this->url->link('extension/module/bhavesh_megamenu', 'module_id='.$module_id.'&'.$token_prefix.'=' . $this->session->data[$token_prefix], true));
			}
        }
		...
		if (!isset($this->request->get['module_id'])) {
			$data['breadcrumbs'][] = array(
				...
				'href' => $this->url->link('extension/module/bhavesh_megamenu', $token_prefix . '=' . $this->session->data[$token_prefix], true)
			);
		} else {
			$data['breadcrumbs'][] = array(
				'text' => $this->language->get('heading_title'),
				'href' => $this->url->link('extension/module/bhavesh_megamenu', $token_prefix . '=' . $this->session->data[$token_prefix] . '&module_id=' . $this->request->get['module_id'], true)
			);
		}	

		$this->load->model($model_module_load);
		$data['modules'] = $this->$model_module_path->getModulesByCode('bhavesh_megamenu');
		if (isset($this->request->post['head_name'])) {
        ...
        if (!isset($this->request->get['module_id'])) {
            $data['action'] = $this->url->link('extension/module/bhavesh_megamenu', $token_prefix . '=' . $this->session->data[$token_prefix], true);
			$data['selectedid'] = 0;       
	   } else {
            $data['action'] = $this->url->link('extension/module/bhavesh_megamenu', $token_prefix . '=' . $this->session->data[$token_prefix] . '&module_id=' . $this->request->get['module_id'], true);			
        ...
        if (isset($_GET['jsonstring'])) {
            if($this->validate()){
                ...
                foreach ($readbleArray as $key => $value) {
                    if (is_array($value)) {
                        $this->model_extension_bhavesh_bhavesh_megamenu->save_rang($value['parentID'], $value['id'], $key);
                    }
                }
				$this->remove_cache();
        ...
        if(isset($_GET['edit'])) {
            ...
            $dane = $this->model_extension_bhavesh_bhavesh_megamenu->getMenu(intval($_GET['edit']));		
			$this->load->model('tool/image');
			...
			if($dane) {
                ...
                $data['list_categories'] = $this->model_extension_bhavesh_bhavesh_megamenu->getCategories($dane['content']['categories']['categories']);
            } else {
                $this->session->data['error_warning'] = 'This menu does not exist!';
                $this->response->redirect(HTTPS_SERVER . 'index.php?route=extension/module/bhavesh_megamenu&'.$token_prefix.'=' . $this->session->data[$token_prefix]);
            }
        }
        elseif (isset($_GET['duplicate']))
        {
            $this->model_extension_bhavesh_bhavesh_megamenu->duplicateMenu($_GET['duplicate']);
            $this->response->redirect(HTTPS_SERVER . 'index.php?route=extension/module/bhavesh_megamenu&'.$token_prefix.'=' . $this->session->data[$token_prefix].'&module_id='.$this->request->get['module_id']);
        }
        elseif (isset($_GET['changestatus']))
        {
            $dane = $this->model_extension_bhavesh_bhavesh_megamenu->getMenu(intval($_GET['changestatus']));
            if ($dane['status']==1)
                $status['status'] =0;
            else
                $status['status'] =1;
            $status['id'] = intval($_GET['changestatus']);
            $this->model_extension_bhavesh_bhavesh_megamenu->UpdatePosition($status);
			$this->remove_cache();
            $this->response->redirect(HTTPS_SERVER . 'index.php?route=extension/module/bhavesh_megamenu&'.$token_prefix.'=' . $this->session->data[$token_prefix].'&module_id='.$this->request->get['module_id']);
        }
        $this->load->model('tool/image');
        ...
        $data['linkremove'] = $this->url->link('extension/module/bhavesh_megamenu&'.$token_prefix.'=' . $this->session->data[$token_prefix]);
        ...
        $this->response->setOutput($this->load->view('extension/module/bhavesh_megamenu', $data));
    }
	...
	public function uninstall() {
        $this->load->model('setting/setting');
        $this->load->model('extension/bhavesh/bhavesh_megamenu');
        $this->model_extension_bhavesh_bhavesh_megamenu->uninstall();
    }
    ...
    public function install() {
        ...
        $this->load->model($model_module_load);
        $this->load->model('extension/bhavesh/bhavesh_megamenu');
		$this->model_extension_bhavesh_bhavesh_megamenu->install();
    }
	
	protected function validate() {
        if (!$this->user->hasPermission('modify', 'extension/module/bhavesh_megamenu')) {
