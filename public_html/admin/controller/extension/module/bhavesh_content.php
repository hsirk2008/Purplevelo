<?php
class ControllerExtensionModuleBhaveshContent extends Controller {
	private $error = array();
	...
	public function index() {
		...
		$this->load->language('extension/module/bhavesh_content');
		
		$this->document->addStyle('view/javascript/bhavesh/bhavesh_content.css');
		...
		$this->document->addStyle('view/javascript/bhavesh/css/jquery-ui.css');
		$this->document->addScript('view/javascript/bhavesh/jquery-ui.js');
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
			if (!isset($this->request->get['module_id'])) {
				$this->$model_module_path->addModule('bhavesh_content', $this->request->post);
			} else {
			...
			if (isset($this->request->post['save']) && $this->request->post['save'] == 'stay' && $this->request->get['module_id']) {
				$this->response->redirect($this->url->link('extension/module/bhavesh_content', $token_prefix . '=' . $this->session->data[$token_prefix] . '&module_id=' . $this->request->get['module_id'], true)); 
			} else {
		...
		if (!isset($this->request->get['module_id'])) {
			$data['breadcrumbs'][] = array(
				...
				'href' => $this->url->link('extension/module/bhavesh_content', $token_prefix . '=' . $this->session->data[$token_prefix], true)
			);
		} else {
			$data['breadcrumbs'][] = array(
				'text' => $this->language->get('heading_title'),
				'href' => $this->url->link('extension/module/bhavesh_content', $token_prefix . '=' . $this->session->data[$token_prefix] . '&module_id=' . $this->request->get['module_id'], true)
			);
		}

		if (!isset($this->request->get['module_id'])) {
			$data['action'] = $this->url->link('extension/module/bhavesh_content', $token_prefix . '=' . $this->session->data[$token_prefix], true);
		} else {
			$data['action'] = $this->url->link('extension/module/bhavesh_content', $token_prefix . '=' . $this->session->data[$token_prefix] . '&module_id=' . $this->request->get['module_id'], true);
		}
		...
		if (isset($this->request->get['import_id']) && ($this->request->server['REQUEST_METHOD'] != 'POST')) {
			$content = file_get_contents(DIR_APPLICATION . 'view/javascript/bhavesh/content_templates/' . $this->request->get['import_id'] . '/content.txt');	
		...
		$templates = glob(DIR_APPLICATION . 'view/javascript/bhavesh/content_templates/*');
		if ($templates) {
		...
		foreach ($templates as $template) {
			...
			'import_url' => $this->url->link('extension/module/bhavesh_content', $token_prefix . '=' . $this->session->data[$token_prefix] . $module_prefix . '&import_id=' . basename($template), true),
			'name'      => file_get_contents(DIR_APPLICATION . 'view/javascript/bhavesh/content_templates/' . basename($template) . '/name.txt')
			);
		}
		$sort_order = array();
		...
		$this->response->setOutput($this->load->view('extension/module/bhavesh_content', $data));
	}

	protected function validate() {
		if (!$this->user->hasPermission('modify', 'extension/module/bhavesh_content')) {
