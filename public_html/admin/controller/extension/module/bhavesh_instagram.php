<?php
class ControllerExtensionModuleBhaveshInstagram extends Controller {
	private $error = array();
	...
	public function index() {
		...
		$this->load->language('extension/module/bhavesh_instagram');
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
			if (!isset($this->request->get['module_id'])) {
				$this->$model_module_path->addModule('bhavesh_instagram', $this->request->post);
			} else {
		...
		if (!isset($this->request->get['module_id'])) {
			$data['breadcrumbs'][] = array(
				...
				'href' => $this->url->link('extension/module/bhavesh_instagram', $token_prefix . '=' . $this->session->data[$token_prefix], true)
			);
		} else {
			$data['breadcrumbs'][] = array(
				'text' => $this->language->get('heading_title'),
				'href' => $this->url->link('extension/module/bhavesh_instagram', $token_prefix . '=' . $this->session->data[$token_prefix] . '&module_id=' . $this->request->get['module_id'], true)
			);			
		}
		
		if (!isset($this->request->get['module_id'])) {
			$data['action'] = $this->url->link('extension/module/bhavesh_instagram', $token_prefix . '=' . $this->session->data[$token_prefix], true);
		} else {
			$data['action'] = $this->url->link('extension/module/bhavesh_instagram', $token_prefix . '=' . $this->session->data[$token_prefix] . '&module_id=' . $this->request->get['module_id'], true);
		}
		...
		$this->response->setOutput($this->load->view('extension/module/bhavesh_instagram', $data));
	}

	protected function validate() {
		if (!$this->user->hasPermission('modify', 'extension/module/bhavesh_instagram')) {
