...
<div id="content">
  <div class="page-header">
    <div class="container-fluid">
      ...
      <a class="btn btn-success" onclick="$('#save').val('stay');$('#form_bhavesh_content').submit();" data-toggle="tooltip" title="<?php echo $button_save_stay; ?>"><i class="fa fa-check"></i></a>
      <?php } ?>
    <button type="submit" form="form_bhavesh_content" data-toggle="tooltip" title="<?php echo $button_save; ?>" class="btn btn-primary"><i class="fa fa-save"></i></button>
  ...
  <div class="container-fluid">
    ...
    <form action="<?php echo $action; ?>" method="post" enctype="multipart/form-data" id="form_bhavesh_content" class="form-horizontal">
...
var template_preview = function(template, name) {
	$.ajax({
		...
		success: function(data) {
			...
			html += '      </div>';
			html += '      <div class="modal-body"><img src="view/javascript/bhavesh/content_templates/' + template + '/preview.png"</div>';
			html += '    </div';
			html += '  </div>';
			html += '</div>';
			$('body').append(html);
...
$(document).delegate('.icon_list', 'click', function(e) {
	...
	$.ajax({
		...
		success: function(data) {
			...
			html += '      </div>';
			html += '      <div class="modal-body" style="padding:30px 0;"><iframe src="view/javascript/bhavesh/icons_list/icon_list.html" width="1240" height="560" frameborder="0" allowtransparency="true"></iframe></div>';
			html += '    </div';
			html += '  </div>';
			html += '</div>';
			$('body').append(html);
...
$(document).delegate('.shortcode_list', 'click', function(e) {
	...
	$.ajax({
		...
		success: function(data) {
			...
			html += '      </div>';
			html += '      <div class="modal-body"><iframe src="view/javascript/bhavesh/shortcode_list/shortcode_list.html" width="1240" height="560" frameborder="0" allowtransparency="true"></iframe></div>';
			html += '    </div';
			html += '  </div>';
			html += '</div>';
			$('body').append(html);
...
$(document).delegate('.overlay_list', 'click', function(e) {
	...
	$.ajax({
		...
		success: function(data) {
			...
			html += '      </div>';
			html += '      <div class="modal-body"><iframe src="view/javascript/bhavesh/overlay_list/overlay_list.html" width="1240" height="560" frameborder="0" allowtransparency="true"></iframe></div>';
			html += '    </div';
			html += '  </div>';
			html += '</div>';
			$('body').append(html);
