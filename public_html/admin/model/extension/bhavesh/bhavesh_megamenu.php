<?php
class ModelExtensionBhaveshBhaveshMegamenu extends Model {
    ...
    public function generate_nestable_list($lang_id) {
        ...
        if (!isset($this->request->get['module_id'])) {
            $action = $this->url->link('extension/module/bhavesh_megamenu&', $token_prefix . '=' . $this->session->data[$token_prefix], true);
        } else {
            $action = $this->url->link('extension/module/bhavesh_megamenu&', $token_prefix . '=' . $this->session->data[$token_prefix] . '&module_id=' . $this->request->get['module_id'], true);
        }
	...
	public function duplicateModule($module_id,$import_module){
		...
		if($parent_menu){
			foreach ($parent_menu as $menu) {
				$dane = $this->model_extension_bhavesh_bhavesh_megamenu->getMenu(intval($menu['id']));
				$dane['module_id'] = $module_id;
				$id_parent_add = $this->model_extension_bhavesh_bhavesh_megamenu->addMenu($dane);
				$subcategories = $this->model_extension_bhavesh_bhavesh_megamenu->getSubMenu(intval($menu['id']));
				if($subcategories){
				foreach ($subcategories as $result) {
					$data = array(
							...
							'content_type' => $result['content_type'],
							'content' => json_decode($result['content'],true),
							'list_categories' => (isset($result['content']['categories']['categories']) && $result['content']['categories']['categories']) ? $this->model_extension_bhavesh_bhavesh_megamenu->getCategories(json_decode($result['content']['categories']['categories'],true)) : '',
						);
						$this->model_extension_bhavesh_bhavesh_megamenu->addMenu($data);
					}
				}
			}
		}
	...
	public function duplicateMenu($id_duplicate){
		$dane = $this->model_extension_bhavesh_bhavesh_megamenu->getMenu(intval($id_duplicate));
		$id_parent_add = $this->model_extension_bhavesh_bhavesh_megamenu->addMenu($dane);
		$subcategories = $this->model_extension_bhavesh_bhavesh_megamenu->getSubMenu(intval($id_duplicate));
		if($subcategories){
			foreach ($subcategories as $result) {
				$data = array(
					'parent_id' => $id_parent_add,
