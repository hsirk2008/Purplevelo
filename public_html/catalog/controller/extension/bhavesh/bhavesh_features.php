<?php
class ControllerExtensionBhaveshBhaveshFeatures extends Controller {
...
public function add_to_cart() {
		...
		if ($product_info) {
			...
			if (!$json) {
				...
				if ($this->config->get('bhavesh_cart_action') == 'redirect_cart') $json['success_redirect'] = $this->url->link('checkout/cart');
				if ($this->config->get('bhavesh_cart_action') == 'redirect_checkout') $json['success_redirect'] = $this->url->link('checkout/checkout', '', true);
...
public function add_to_compare() {
		...
		if ($product_info) {
			...
			if ($this->config->get('bhavesh_compare_action') == 'redirect') $json['success_redirect'] = $this->url->link('product/compare');
...
public function add_to_wishlist() {
		...
		if ($this->config->get('bhavesh_wishlist_action') == 'redirect') $json['success_redirect'] = $this->url->link('account/wishlist');
...
// Search
public function bhavesh_search() {
	
		$this->load->language('bhavesh/bhavesh_theme');
		
		$data['bhavesh_text_search'] = $this->language->get('bhavesh_text_search');
		$data['bhavesh_text_category'] = $this->language->get('bhavesh_text_category');		
		...
		if ($this->config->get('theme_default_directory') == 'bhavesh') {
			if ($this->config->get('bhavesh_header') == 'header6') {
				return $this->load->view('common/bhavesh_search_full', $data);
			} else {
				return $this->load->view('common/bhavesh_search', $data);
			}
		}
}
	...
	$this->load->language('bhavesh/bhavesh_theme');
		...
		if ($this->request->server['REQUEST_METHOD'] == 'POST') {
			if(!filter_var($this->request->post['email'], FILTER_VALIDATE_EMAIL)){
				$json['error'] = $this->language->get('bhavesh_subscribe_invalid_email');
			}
			if (!isset($json['error'])) {
				$this->load->model('extension/bhavesh/newsletter'); 
			if($this->model_extension_bhavesh_newsletter->checkRegistered($this->request->post)){
			   	$this->model_extension_bhavesh_newsletter->UpdateRegistered($this->request->post,1);
				$json['success'] = $this->language->get('bhavesh_subscribe_success');
			} else if ($this->model_extension_bhavesh_newsletter->checkExist($this->request->post)){
				$json['error'] = $this->language->get('bhavesh_subscribe_email_exist');
			} else {
				$this->model_extension_bhavesh_newsletter->subscribe($this->request->post);
				$json['success'] = $this->language->get('bhavesh_subscribe_success');
		   	} 
		  }
		}
	...
	$this->load->language('bhavesh/bhavesh_theme');
		...
		if ($this->request->server['REQUEST_METHOD'] == 'POST') {
			...
			if (!isset($json['error'])) {
				$this->load->model('extension/bhavesh/newsletter'); 
			if($this->model_extension_bhavesh_newsletter->checkRegistered($this->request->post)){
			   	$this->model_extension_bhavesh_newsletter->UpdateRegistered($this->request->post,0);
				$json['success'] = $this->language->get('bhavesh_unsubscribe_unsubscribed');
			} else if (!$this->model_extension_bhavesh_newsletter->checkExist($this->request->post)){
				$json['error'] = $this->language->get('bhavesh_unsubscribe_not_found');
			} else {
				$this->model_extension_bhavesh_newsletter->unsubscribe($this->request->post);
				$json['success'] = $this->language->get('bhavesh_unsubscribe_unsubscribed');
		   	} 
		  }
		}
...
public function bhavesh_captcha() {
		$num1=rand(2,6);
		$num2=rand(2,6);
...
// Contact Form	
public function bhavesh_send_message () {
	$this->load->language('bhavesh/bhavesh_theme');
	$json = array();
	if ($this->request->server['REQUEST_METHOD'] == 'POST') {
		...
		if ((utf8_strlen($this->request->post['text']) < 5) || (utf8_strlen($this->request->post['text']) > 1000)) {
			$json['error'] = $this->language->get('bhavesh_error_message');
		}
		...
		if (!isset($json['error'])) {
			...
			$mail->setSubject(html_entity_decode(sprintf($this->language->get('bhavesh_email_subject'), $this->request->post['name']), ENT_QUOTES, 'UTF-8'));
			$mail->setText($this->request->post['text']);
			$mail->send();
			$json['success'] = $this->language->get('bhavesh_text_success_form');
		}
	}
	$this->response->addHeader('Content-Type: application/json');
	$this->response->setOutput(json_encode($json));
...
// Popup
public function bhavesh_popup() {
		$lang_id = $this->config->get('config_language_id');
		$data['module'] = '99';
		$this->load->language('bhavesh/bhavesh_theme');
		$data['bhavesh_subscribe_email'] = $this->language->get('bhavesh_subscribe_email');
		$data['bhavesh_subscribe_btn'] = $this->language->get('bhavesh_subscribe_btn');
		$data['popup_width'] = $this->config->get('bhavesh_popup_note_w');
		$data['popup_height'] = $this->config->get('bhavesh_popup_note_h');
		...
		if (is_file(DIR_IMAGE . $this->config->get('bhavesh_popup_note_img'))) {
			$data['img'] = $server . 'image/' . $this->config->get('bhavesh_popup_note_img');
		} else {
			$data['img'] = '';
		}
		
		$content_block = $this->config->get('bhavesh_popup_note_block');
$data['popup_content_block'] = html_entity_decode(str_replace('{signup}',$this->load->view('extension/module/content_widgets/subscribe_field', $data),$content_block[$this->config->get('config_language_id')]), ENT_QUOTES, 'UTF-8');
		$content_title = $this->config->get('bhavesh_popup_note_title');
