...
class ControllerExtensionModuleBhaveshCategories extends Controller {
	public function index($setting) {
		...
		$data['bhavesh_text_view_products'] = $this->language->get('bhavesh_text_view_products');
		...
		if (!empty($setting['category'])) {
		  ...
		  foreach ($setting['category'] as $category) {
			...
			$data['categories'][] = array(
				...
				'products'    => $this->model_catalog_product->getTotalProducts($filter_data_top) . $this->language->get('bhavesh_text_products'),
		...
		return $this->load->view('extension/module/bhavesh_categories', $data);
	}
}