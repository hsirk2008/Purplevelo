...
class ControllerExtensionModuleBhaveshContent extends Controller {
	public function index($setting) {
		...
		$data['bhavesh_unsubscribe_btn'] = $this->language->get('bhavesh_unsubscribe_btn');
		$data['bhavesh_text_name'] = $this->language->get('bhavesh_text_name');
		$data['bhavesh_text_email'] = $this->language->get('bhavesh_text_email');
		$data['bhavesh_text_message'] = $this->language->get('bhavesh_text_message');
		$data['bhavesh_text_captcha'] = $this->language->get('bhavesh_text_captcha');
		$data['bhavesh_text_submit'] = $this->language->get('bhavesh_text_submit');
		...
		if ($setting['b_setting']['block_bgv']) {
			$this->document->addScript('catalog/view/theme/bhavesh/js/jquery.tuber.js');
		}
		
		// Equal Height
		if ($setting['c_setting']['eh']) {
			$this->document->addScript('catalog/view/theme/bhavesh/js/jquery.matchHeight.min.js');
		}
		...
		$data['block_style'] .= "background-repeat:" . $setting['b_setting']['bg_repeat'] . ";";
			...
			$this->document->addScript('catalog/view/theme/bhavesh/js/parallax.min.js');
			}
		}
		...
		if (isset($setting['columns'])) {
            ...
            foreach($setting['columns'] as $column) {
				...
				if ($column['type'] != "testimonial") {
					...
					$results = $this->model_extension_bhavesh_testimonial->getTestimonials($column['data1']);
		...
		return $this->load->view('extension/module/bhavesh_content', $data);
	}
}