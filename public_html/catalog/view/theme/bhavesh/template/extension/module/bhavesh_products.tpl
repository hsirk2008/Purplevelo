...
<?php if ($block_title) { ?>
<!-- Block Title -->
<div class="widget-title fadeInUp animated">
<?php if ($title_preline) { ?><p class="pre-line"><?php echo $title_preline; ?></p><?php } ?>
<?php if ($title) { ?> 
<p class="main-title fadeInUp animated"><span><?php echo $title; ?></span></p>
<p class="widget-title-separator fadeInUp animated"><i class="icon-line-cross"></i></p>
<?php } ?>
<?php if ($title_subline) { ?>
<p class="sub-line fadeInUp animated"><span><?php echo $title_subline; ?></span></p>
<?php } ?>
</div>
<?php } ?>
<?php if (count($tabs) >= 2) { ?>
    ...
    <div class="grid-holder grid<?php echo $columns; ?> prod_module<?php echo $module; ?> <?php if ($carousel) echo "carousel"; ?> <?php if ($carousel_a && $rows > 1) echo "sticky-arrows"; ?>">
        <?php foreach ($tab['products'] as $product) { ?>
            ...
            <?php if (($product['quantity'] < 1) && ($stock_badge_status)) { ?>
                ...
                <?php $button_cart = $bhavesh_text_out_of_stock; ?>
             <?php } else { ?>
            ...
            <a class="icon is_quickview hidden-xs" onclick="quickview('<?php echo $product['product_id']; ?>');" data-toggle="tooltip" data-placement="<?php echo $tooltip_align; ?>" data-title="<?php echo $bhavesh_button_quickview; ?>"><span class="icon-magnifier-add"></span></a>
            </div> <!-- .icons-wrapper -->
            </div><!-- .image ends -->
            <div class="caption">
            ...
            <a class="btn catalog_hide <?php if ($bhavesh_list_style == '6') { echo 'btn-contrast'; } else { echo 'btn-outline';} ?>" onclick="cart.add('<?php echo $product['product_id']; ?>', '<?php echo $product['minimum']; ?>');"><span class="global-cart"></span><?php echo $button_cart; ?></a>
            </div><!-- .price-wrapper -->
            ...
            <a class="icon is_quickview link-hover-color" onclick="quickview('<?php echo $product['product_id']; ?>');"><span class="icon-magnifier-add"></span> <?php echo $bhavesh_button_quickview; ?></a>
            </div><!-- .plain-links-->
            </div><!-- .caption-->
