	...
	$(live_search.selector).autocomplete({
		'source': function(request, response) {
			...
			if (filter_name.length == 0) {
				...
				$('.result-text').html();
				var html = '';
				html +=	'<tr><td style="text-align:center;padding:20px 10px 10px"><span class="bhavesh-spinner"></span></td></tr>';
				$('.live-search .table.products').html(html);
				$('.live-search').css('display','block');

				$.ajax({
					url: 'index.php?route=extension/bhavesh/live_search&filter_name=' +  encodeURIComponent(filter_name),
					dataType: 'json',
					type: 'post',
					success: function(result,json) {
						...
						if (!$.isEmptyObject(products)) {
							var show_image = true;
							var show_price = true;
							$('.result-text').html('<a href="'+result.search_url+'&search='+filter_name+'" class="view-all-results">'+result.bhavesh_text_view_all+'('+result.total+')</a>');
							...
							var html = '';
							html +=	result.bhavesh_text_no_result;
